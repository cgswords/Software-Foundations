...
Proof.
  ...
  bevalR_cases (induction H) SCase;
    try reflexivity;
    try (simpl; rewrite aeval_iff_aevalR in *; subst; reflexivity).
    SCase "E_BNot".
  ...
  bevalR_cases (induction e) SCase;
    try (intros; simpl in H; rewrite <- H; constructor);
    try (subst; rewrite aeval_iff_aevalR; reflexivity).
  SCase "E_BNot".
...
Inductive no_whilesR: com -> Prop :=
  ...
  | nw_assn : forall x a, no_whilesR (x ::= a)
...
Proof.
  ...
  Case "->".
    ...
    induction c; 
      try constructor;
      try (inversion H; apply andb_true_iff in H1; destruct H1; try auto).  
  Case "<-".
    intros.
    induction H; 
      try (reflexivity);
      try (simpl; assert ((no_whiles c1 = true) /\ (no_whiles c2 = true)));
      try (split; assumption);
      try (apply andb_true_intro in H1; assumption).
Qed.
...
Proof.
  ...
  Case "SEQ".
    ...
    auto.
    inversion H1 as [st' c1eval].
    assert (exists st'', c2 / st' || st'').
    auto.
    inversion H2 as [st'' c2eval].
    exists st''.
  ...
  Case "IF".  
    ...
    destruct cond.
    destruct IHno_whilesR1 with (st:=st).
    exists x.
    apply E_IfTrue.
    auto.
    auto.
    destruct IHno_whilesR2 with (st:=st).
    exists x.
    apply E_IfFalse.
    auto.
    auto.
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  | (SLoad x)::p => s_execute st (st x :: stack) p
  | SPlus::p     => match stack with
                    | (n::m::s) => s_execute st ((n + m)::s) p
                    | s         => s_execute st s p
                    end
  | SMinus::p    => match stack with
                    | (n::m::s) => s_execute st ((m - n)::s) p
                    | s         => s_execute st [] p
                    end
...
Theorem s_exec : forall st stack e1 e2, 
...
Proof.
  ...
  simpl. reflexivity.
  destruct a;
    try (simpl; intros; rewrite IHe1);
    try (reflexivity);
    try (destruct stack; repeat auto).
  destruct stack; repeat auto.
  destruct stack; repeat auto.
  destruct stack; repeat auto.
Qed.

Theorem s_comp : forall st stack e,
...
Proof.
  ...
  induction e; 
    ...
    try (simpl; intros; 
         rewrite s_exec; rewrite IHe1; 
         rewrite s_exec; rewrite IHe2; 
         simpl; 
         try (rewrite plus_comm); try (rewrite mult_comm);
         reflexivity).
Qed.
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  ...
  | E_Break : forall st,
      CBreak / st || SBreak / st
  | E_Assn : forall st i a n,
      aeval st a = n ->
      CAss i a / st || SContinue / update st i n
  ...
  | E_Seq_Continue : forall c1 c2 st st' st'' s,
      ...
      c2 / st' || s / st'' ->
      (c1 ;; c2) / st || s / st''
  | E_IfTrue : forall b c1 c2 st st' s,
      beval st b = true ->
      c1 / st || s / st' ->       
      (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_IfFalse : forall b c1 c2 st st' s,
      beval st b = false ->
      c2 / st || s / st' ->
      (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_WhileEnd : forall b st c,
...
Lemma break_sbreak : forall c st st' s,
                       (BREAK;; c) / st || s / st' ->
                       s = SBreak.
Proof.
  intros.
...
Theorem while_false_ignore : forall b c st st' s,
   beval st b = false ->
   (WHILE b DO c END) / st || s / st' -> st = st'.
Proof.
  intros.
  inversion H0.
  subst. reflexivity.
...
Proof.
  intros.
  apply E_WhileBreak with (st':=st') (c:=c) in H.
  assumption.
  assumption.
Qed.
...
Proof.
  ...
  ceval_cases (induction E1) Case;
    try (intros; inversion E2; split; reflexivity);
    try (intros; inversion E2; subst; split; reflexivity).
  Case "E_Seq_Break".
  intros.
  inversion E2; subst.
  SCase "SBreak".
  ...
  remember E2 as E3.
  clear HeqE3.
  apply while_false_ignore in E3.
  apply while_continue in E2.
  symmetry in E2.
  split; assumption.
  assumption.
  Case "E_WhileBreak".
  ...
  remember H as H2.
  clear HeqH2.
  apply while_stops_on_break with (st':=st') (c:=c) in H.
  inversion E2; subst.
...
Theorem beval_opt_eq : forall st b,
